...
public:
    ...
    // This function deletes a node from a doubly linked list (DLL) at a given position.
    ...
    void deleteFromPosition(int k)
    {
        ...
        while (current != k) // Traverse the DLL to reach the desired position.
        {
            temp = temp->next;
            current++;
        }

        // Update the previous and next pointers of the nodes adjacent to the node to be deleted.
        temp->prev->next = temp->next;
        temp->next->prev = temp->prev;

        temp->prev = NULL; // Update the prev and next pointers of the deleted node to NULL.
        temp->next = NULL;
